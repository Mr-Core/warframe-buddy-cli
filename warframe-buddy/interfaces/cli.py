...
def interactive_search(search_engine) -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            matching_items = [item for item in all_items if item_lower in item.lower()]
            ...
            if len(matching_items) > 1:
                ...
                selection = input(
                    "\nSelect item number (or press Enter for all): "
                ).strip()
            ...
            if search_results:
                ...
                source_type = (
                    input("\nEnter source type (or press Enter for all): ")
                    .strip()
                    .lower()
                )
                ...
                if source_type:
                    ...
                    if actual_source_type:
                        ...
                        filter_display = actual_source_type.replace(
                            " Location Rewards", ""
                        )
                    else:
...
def display_summary(summary: dict) -> None:
    ...
    if summary["best_source"]:
        ...
        if best["source_type"] == "Missions":
            if "rotation" in best:
                print(
                    f"   Location: {best.get('planet_name')} / {best.get('mission_name')} / {best.get('mission_type')} - Rotation: {best.get('rotation')}"
                )
            else:
                print(
                    f"   Location: {best.get('planet_name')} / {best.get('mission_name')} / {best.get('mission_type')}"
                )

        elif best["source_type"] == "Relics":
            print(
                f"      Relic: {best.get('relic_tier')} {best.get('relic_name')} {best.get('relic_refinement')}"
            )

        elif best["source_type"] == "Bounties":
            if "rotation" in best:
                print(
                    f"   Location: {best.get('planet_name')} / {best.get('mission_name')} / {best.get('bounty_name')} / {best.get('bounty_level')} / Rotation {best.get('rotation')} / {best.get('stage')}"
                )
            else:
                print(
                    f"   Location: {best.get('planet_name')} / {best.get('mission_name')} / {best.get('bounty_name')} / {best.get('bounty_level')} / {best.get('stage')}"
                )
    ...
    print("\nBreakdown legend:")
    print(
        "  - Missions: Planet / Mission name / Mission type / Rotation -> Drop chance"
    )
    print("  - Relics: Relic tier / Relic name / Relic refinement -> Drop chance")
    print(
        "  - Bounties: Planet / Mission name / Bounty name / Bounty level / Rotation / Stage -> Drop chance"
    )
    ...
    if summary["missions"]:
        ...
        for mission in summary["missions"][:10]:
            if "rotation" in mission:
                print(
                    f"  • {mission['planet']} / {mission['mission']} / {mission['type']} / {mission['rotation']} -> {mission['chance']:.1%}"
                )
            else:
                print(
                    f"  • {mission['planet']} / {mission['mission']} / {mission['type']} -> {mission['chance']:.1%}"
                )
    ...
    if summary["relics"]:
        ...
        for relic in summary["relics"][:10]:
            print(
                f"  • {relic['tier']} {relic['name']} {relic['refinement']} -> {relic['chance']:.1%}"
            )
    ...
    if summary["bounties"]:
        ...
        for bounty in summary["bounties"][:10]:
            if bounty.get("rotation"):
                print(
                    f"  • {bounty['planet']} / {bounty['mission']} / {bounty['name']} / {bounty['level']} / Rotation {bounty['rotation']} / {bounty['stage']} -> {bounty['chance']:.1%}"
                )
            else:
                print(
                    f"  • {bounty['planet']} / {bounty['mission']} / {bounty['name']} / {bounty['level']} / {bounty['stage']} -> {bounty['chance']:.1%}"
                )
